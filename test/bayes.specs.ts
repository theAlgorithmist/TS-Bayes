...
import {TSMT$FrequencyTable} from "../src/FrequencyTable";
import {TSMT$Bayes         } from "../src/Bayes";
...
let spamTestClone: TSMT$FrequencyTable;

// Test Suites
describe('Frequency Table Tests: TSMT$FrequencyTable', () =>
{
  it('properly constructs a new Frequency Table', () =>
  {
    const table: TSMT$FrequencyTable = new TSMT$FrequencyTable();
  
    expect(table.rowCount).to.equal(0);
    expect(table.colCount).to.equal(0);
    expect(table.rowTotals.length).to.equal(0);
    expect(table.columnTotals.length).to.equal(0);
    expect(table.rowLabels.length).to.equal(0);
    expect(table.columnLabels.length).to.equal(0)
  });
  ...
  {
    expect(spamTestTable.rowCount).to.equal(2);
    expect(spamTestTable.colCount).to.equal(8);

    const rowTotals: Array<number> = spamTestTable.rowTotals;
    const colTotals: Array<number> = spamTestTable.columnTotals;

    expect(rowTotals.length).to.equal(2);
    expect(colTotals.length).to.equal(8);
    expect(rowTotals[0]).to.equal(20);
    expect(rowTotals[1]).to.equal(80);

    expect(colTotals[0]).to.equal(5);
    expect(colTotals[1]).to.equal(95);
    expect(colTotals[2]).to.equal(24);
    expect(colTotals[3]).to.equal(76);
    expect(colTotals[4]).to.equal(8);
    expect(colTotals[5]).to.equal(91);
    expect(colTotals[6]).to.equal(35);
    expect(colTotals[7]).to.equal(65);
  });
  ...
  it('properly clones a frequency table', () =>
  {
    ...
    expect(spamTestClone.rowCount).to.equal(2);
    expect(spamTestClone.colCount).to.equal(8);
  ...
  it('applies proper table validation', () =>
  {
    ...
    table.setupTable(rows, cols);
    validate = table.validate();
    expect(validate).to.be.false;  // singleton column

    cols = ['col1', 'col2'];
    table.setupTable(rows, cols);

    validate = table.validate();
    expect(validate).to.be.false;  // no row totals
    ...
    validate = table.validate();
    expect(validate).to.be.false;  // bad cell data
    ...
    validate = table.validate();
    expect(validate).to.be.false;  // cell frequency exceeds row count
    ...
    validate = table.validate();
    expect(validate).to.be.true;  // finally! a good one !
  });
  ...
  it('table accessor preserves immutability of internal data', () =>
  {
    ...
    expect(tableData[0][0]).to.equal(43);

    // even more medieval
    col1Data.length = 0;
    col2Data.length = 0;

    expect(tableData[0].length).to.equal(7);
    expect(tableData[1].length).to.equal(7);
  });
});

describe('Naive Bayes: TSMT$Bayes', () =>
{
  it('properly constructs a new Bayes instance', () =>
  {
    const bayes: TSMT$Bayes = new TSMT$Bayes();

    expect(bayes).to.not.be.undefined;
  });
  ...
  it('works properly with a singleton predictor', () =>
  {
    ...
    expect(Math.abs(result - 0.6) < 0.001).to.be.true;
  });
  ...
  it('works properly with multiple predictors #2', () =>
  {
    ...
    expect(Math.abs(result - 0.0818) < 0.001).to.be.true;
  });
  ...
  it('works properly with multiple predictors #3', () => {
    ...
    expect(Math.abs(pNotSpam - 0.0021) < 0.001).to.be.true;
    ...
    expect(Math.abs(probSpam - 0.85076) < 0.0001).to.be.true;
    expect(Math.abs(probNotSpam - 0.14924) < 0.0001).to.be.true;
  });
});